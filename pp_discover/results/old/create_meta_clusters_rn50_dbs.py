...
# Set device (should default to the first available device in the visible list, e.g., GPU 1)
...
def cluster_images():
    ...
    print("\nFeature extraction complete. Performing DBSCAN clustering...")

    # Convert feature list to a numpy array for clustering
    features_array = np.array(features_list)
    
    # Apply DBSCAN with a specified eps (distance threshold) and min_samples
    dbscan = DBSCAN(eps=2, min_samples=3).fit(features_array)
    labels = dbscan.labels_
    
    # Get unique cluster labels and count
    ...
    for label in unique_labels:
        if label == -1:
            # -1 label indicates noise (unclustered points)
            cluster_dir = os.path.join(dest_dir, 'noise')
        else:
...
if __name__ == "__main__":
    print("Starting image clustering based on visual similarity using PyTorch and ResNet50 on GPU with DBSCAN...")
    cluster_images()
